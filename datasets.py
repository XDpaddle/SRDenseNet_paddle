...
class TrainDataset(paddle.io.Dataset):
    ...
    def random_crop(self, lr, hr):
        ...
        hr = hr[:, y * self.scale:y * self.scale + self.patch_size * self.
            scale, x * self.scale:x * self.scale + self.patch_size * self.scale
            ]
        return lr, hr

    def __getitem__(self, idx):
        with h5py.File(self.h5_file, 'r') as f:
            lr = np.expand_dims(f['lr'][str(idx)][:].astype(np.float32), 0
                ) / 255.0
            hr = np.expand_dims(f['hr'][str(idx)][:].astype(np.float32), 0
                ) / 255.0
